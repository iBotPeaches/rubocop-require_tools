...
module RuboCop
  module RequireTools
    ...
    class State
      ...
      # This method intentionally contains several conditional branches to mirror Ruby's
      # constant lookup semantics (top-level, nested, locally defined, and alias resolution).
      # Extracting the logic would hurt readability more than it would help, so we allow
      # a slightly higher perceived complexity here.
      ...
      def access_const(const_name: nil, local_only: false)
        ...
        # If the access is made through a known alias (e.g., DisplayType or DisplayType::Something),
        # consider it accessible as soon as the alias has been established in this file, even if the
        # target constant isn't loadable in the analysis environment. This prevents false positives
        # when the required library isn't available to the analyzer but the code has a proper require.
        alias_prefix = self.const_aliases.keys.find { |an| name == an || name.start_with?("#{an}::") }
        if alias_prefix && name == alias_prefix && self.defined_constants.include?(alias_prefix)
          # The alias name itself (as a namespace) is considered defined once registered via const_aliased.
          # Nested accesses (alias::X) still require the target to be resolvable or locally defined.
          return true
        end
